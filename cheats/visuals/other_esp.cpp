...
void otheresp::indicators()
{
	...
	if (g_cfg.esp.indicators[INDICATOR_SAFE_POINTS] && key_binds::get().get_key_bind_state(KB_SAFEPOINTS) && !weapon->is_non_aim())
		m_indicators.push_back(m_indicator("SAFE POINTS", Color(130, 170, 20)));

	if (g_cfg.esp.indicators[INDICATOR_BODY_AIM] && key_binds::get().get_key_bind_state(KB_BODYAIM) && !weapon->is_non_aim())
...
void otheresp::automatic_peek_indicator()
{
	...
	if (!weapon->is_non_aim() && key_binds::get().get_key_bind_state(KB_AUTOPEEK) || alpha)
	{
		if (!weapon->is_non_aim() && key_binds::get().get_key_bind_state(KB_AUTOPEEK))
