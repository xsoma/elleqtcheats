...
float antiaim::get_yaw(CUserCmd* m_pcmd)
{
	...
	{	
		...
		if (g_cfg.antiaim.desync != 2 && (g_cfg.antiaim.flip_desync.key <= KEY_NONE || g_cfg.antiaim.flip_desync.key >= KEY_MAX))
			...
			flip = key_binds::get().get_key_bind_state(KB_FLIPDESYNC);
...
{	
	...
	if (g_ctx.globals.fakeducking && m_clientstate()->m_iChockedCommands > 12)
		return false;

	if (!g_ctx.globals.fakeducking && m_clientstate()->m_iChockedCommands > 14)
	{
