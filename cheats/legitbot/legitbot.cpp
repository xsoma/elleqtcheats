...
void legit_bot::do_smooth(const Vector& currentAngle, const Vector& aimAngle, Vector& angle)
{
	...
	static auto recoil_scale = m_cvar()->FindVar("weapon_recoil_scale");
	aim_punch = g_ctx.local()->m_aimPunchAngle() * recoil_scale->GetFloat();
...
void legit_bot::createmove(CUserCmd* cmd)
{
	...
	if (target_switch_delay > m_globals()->m_realtime)
	{
		...
		if ((key_binds::get().get_key_bind_state(KB_LEGITKEY) || key_binds::get().get_key_bind_state(KB_AUTOFIRE)) && !g_cfg.legitbot.weapon[g_ctx.globals.current_weapon].rcs_type)
			do_rcs(cmd);
	...
	// legitbot key check
	if (!key_binds::get().get_key_bind_state(KB_LEGITKEY) && !key_binds::get().get_key_bind_state(KB_AUTOFIRE))
	{
		autofire_delay = 0;
	...
	if (!g_ctx.local()->m_iShotsFired() && key_binds::get().get_key_bind_state(KB_AUTOFIRE) && autofire_delay < g_cfg.legitbot.autofire_delay)
	{
		autofire_delay += 1;
		return;
	}
	...
	if (key_binds::get().get_key_bind_state(KB_AUTOFIRE) && g_ctx.globals.weapon->can_fire(true) && hitchanced)
		cmd->m_buttons |= IN_ATTACK;
