...
void fakelag::Fakelag(CUserCmd* m_pcmd)
{
	...
	auto on_land = (!(engineprediction::get().backup_data.flags & FL_ONGROUND) && g_ctx.local()->m_fFlags() & FL_ONGROUND) || m_pcmd->m_buttons & IN_ATTACK;
	...
	if (g_ctx.local()->m_fFlags() & FL_ONGROUND && engineprediction::get().backup_data.flags & FL_ONGROUND && !m_gamerules()->m_bIsValveDS() && key_binds::get().get_key_bind_state(KB_FAKEDUCK)) //-V807
	{
		max_choke = 14;
	...
	{
		if (g_cfg.ragebot.enable && g_ctx.globals.current_weapon != -1 && !g_ctx.globals.exploits && g_cfg.antiaim.fakelag && g_cfg.antiaim.fakelag_enablers[FAKELAG_PEEK] && g_cfg.antiaim.triggers_fakelag_amount > 6 && !started_peeking && velocity >= 5.0f && !on_land)
		{
		...
		if (!g_ctx.globals.exploits && g_cfg.antiaim.fakelag && g_cfg.antiaim.fakelag_enablers[FAKELAG_PEEK] && started_peeking && !on_land)
		{
			if (choked < max_choke)
				g_ctx.send_packet = false;
			else
		...
		else if (!g_ctx.globals.exploits && g_cfg.antiaim.fakelag && velocity >= 5.0f && g_ctx.globals.slowwalking && g_cfg.antiaim.fakelag_enablers[FAKELAG_SLOW_WALK] && !on_land)
		{
			if (choked < max_choke)
				g_ctx.send_packet = false;
			else
		...
		else if (!g_ctx.globals.exploits && g_cfg.antiaim.fakelag && velocity >= 5.0f && !g_ctx.globals.slowwalking && g_ctx.local()->m_fFlags() & FL_ONGROUND && engineprediction::get().backup_data.flags & FL_ONGROUND && g_cfg.antiaim.fakelag_enablers[FAKELAG_MOVE] && !on_land)
		{
			if (choked < max_choke)
				g_ctx.send_packet = false;
			else
		...
		else if (!g_ctx.globals.exploits && g_cfg.antiaim.fakelag && !g_ctx.globals.slowwalking && !(g_ctx.local()->m_fFlags() & FL_ONGROUND && engineprediction::get().backup_data.flags & FL_ONGROUND) && g_cfg.antiaim.fakelag_enablers[FAKELAG_AIR] && !on_land)
		{
			if (choked < max_choke)
				g_ctx.send_packet = false;
			else
		...
		else if (!g_ctx.globals.exploits && g_cfg.antiaim.fakelag && !on_land)
		{
