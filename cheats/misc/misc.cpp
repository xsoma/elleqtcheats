...
void misc::AutoCrouch(CUserCmd* cmd)
{
	...
	if (!key_binds::get().get_key_bind_state(KB_FAKEDUCK))
	{
		g_ctx.globals.fakeducking = false;
		return;
	}

	if (!g_ctx.globals.fakeducking && m_clientstate()->m_iChockedCommands != 7)
		return;

	if (m_clientstate()->m_iChockedCommands >= 7)
...
{
	if (!g_ctx.globals.weapon->is_non_aim() && key_binds::get().get_key_bind_state(KB_AUTOPEEK))
	{
...
void misc::ragdolls()
{
	...
	for (auto i = 1; i <= m_entitylist()->GetHighestEntityIndex(); ++i)
	{
		...
		if (g_cfg.misc.ragdolls == 1)
			ragdoll->m_vecForce().z = 800000;
		else if (g_cfg.misc.ragdolls == 2)
		{
			ragdoll->m_vecForce() *= -95000;
			ragdoll->m_vecForce().z = 500000;
		}
