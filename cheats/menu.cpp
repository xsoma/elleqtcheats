...
void c_menu::menu_setup(ImGuiStyle& style) //-V688
{
	...
	ImGui::SetNextWindowBgAlpha(min(style.Alpha, 1.f)); // window bg alpha setup
...
{
	ImGui::PushFont(c_menu::get().root_large);
...
void c_menu::draw_semitabs(const char* labels[], int count, int& tab, ImGuiStyle& style)
{ 
	ImGui::BeginGroup();
	...
	{
		// switch current tab
		if (ImGui::ContentTab(labels[i], tab == i, ImVec2(90, category_height)))
			tab = i;
...
// SEMITABS FUNCTIONS
void c_menu::draw_tabs_default()
{
	const char* default_sections[1] = { crypt_str("General") };
	draw_semitabs(default_sections, 1, default_section, ImGui::GetStyle());
}
...
void c_menu::draw_ragebot(int child)
{
	if (!rage_section)
	{
		if (!child)
		{
			...
			tab_start();
			ImGui::BeginChild(crypt_str("##RAGE1_FIRST"), ImVec2(child_width, 0));
			{
		...
		else
		{
			...
			tab_start();
			ImGui::BeginChild(crypt_str("##RAGE2_FIRST"), ImVec2(child_width, 0));
			{
	...
	else
	{
		...
		if (!child)
		{
			...
			ImGui::BeginChild(crypt_str("##RAGE1_SECOND"), ImVec2(child_width, 0));
			{
				ImGui::Spacing();
		...
		else
		{
			...
			ImGui::BeginChild(crypt_str("##RAGE2_SECOND"), ImVec2(child_width, 0));
			{
...
void c_menu::draw_visuals(int child)
{
	if (visuals_section != 2)
	{
		...
		if (!child)
		{
			if (!visuals_section)
			{
				...
				tab_start();
				ImGui::BeginChild(crypt_str("##VISUAL1_FIRST"), ImVec2(child_width, 0));
				{
			...
			else
			{
				...
				tab_start();
				ImGui::BeginChild(crypt_str("##VISUAL2_FIRST"), ImVec2(child_width, 0));
				{
		...
		else
		{
			if (!visuals_section)
			{
				...
				tab_start();
				ImGui::BeginChild(crypt_str("##VISUAL1_SECOND"), ImVec2(child_width, 0));
				{
			...
			else
			{
				...
				tab_start();
				ImGui::BeginChild(crypt_str("##VISUAL2_SECOND"), ImVec2(child_width, 0));
				{
	...
	else if (child == -1)
	{
		...
		ImGui::BeginChild(crypt_str("##SKINCHANGER__TAB"), ImVec2(686 * dpi_scale, 0)); // first functional child
		{
			...
			tab_start();
			ImGui::BeginChild(crypt_str("##SKINCHANGER__CHILD"), ImVec2(686 * dpi_scale, 0));
			{
...
void c_menu::draw_players(int child)
{
	...
	if (!child)
	{
		...
		tab_start();
		ImGui::BeginChild(crypt_str("##ESP1_FIRST"), ImVec2(child_width, 0));
		{
	...
	else
	{
		...
		tab_start();
		ImGui::BeginChild(crypt_str("##ESP2_SECOND"), ImVec2(child_width, 0));
		{
			ImGui::Spacing();
			if (player == LOCAL)
...
{
	const char* ragdoll_types[] = { "Default", "Gravity", "Forcing" };
	...
	if (!child)
	{
		if (!misc_section)
		{
			...
			tab_start();
			ImGui::BeginChild(crypt_str("##MISC1_FIRST"), ImVec2(child_width, 0));
			{
				...
				draw_combo(crypt_str("Ragdolls changer"), g_cfg.misc.ragdolls, ragdoll_types, ARRAYSIZE(ragdoll_types));
		...
		else
		{
			...
			ImGui::BeginChild(crypt_str("##MISC1_SECOND"), ImVec2(child_width, 0));
			{
	...
	else
	{
		if (!misc_section)
		{
			...
			tab_start();
			ImGui::BeginChild(crypt_str("##MISC2_FIRST"), ImVec2(child_width, 0));
			{
				...
				draw_multicombo(crypt_str("Walkbot"), g_cfg.misc.walkbot, walkbot_s, ARRAYSIZE(walkbot_s), preview);
				draw_keybind(crypt_str("Create point"), &g_cfg.misc.walkbot_key, crypt_str("##WALKBOT__HOTKEY"));
				...
				if (ImGui::CustomButton(crypt_str("Save path"), crypt_str("##SAVE__PATH_BUT"), ImVec2(291 * dpi_scale, 26 * dpi_scale)))
					walkbot::get().file(addictname, file_overwrite);

				if (ImGui::CustomButton(crypt_str("Load path"), crypt_str("##LOAD__PATH_BUT"), ImVec2(291 * dpi_scale, 26 * dpi_scale)))
					walkbot::get().file(addictname, file_load);

				if (ImGui::CustomButton(crypt_str("Clear path"), crypt_str("##DELETE__POINTS"), ImVec2(291 * dpi_scale, 26 * dpi_scale)))
		...
		else
		{
			...
			tab_start();
			ImGui::BeginChild(crypt_str("##MISC2_SECOND"), ImVec2(child_width, 0));
			{
...
void c_menu::draw_settings(int child)
{
	...
	ImGui::BeginChild(crypt_str("##CONFIGS__FIRST"), ImVec2(686 * dpi_scale, 0));
	{
		ImGui::BeginChild(crypt_str("##CONFIGS__FIRST_POSHELNAHUI"), ImVec2(686 * dpi_scale, 0));
...
void c_menu::draw_lua(int child)
{
	if (!child)
	{
		...
		ImGui::BeginChild(crypt_str("##LUA_FIRST"), ImVec2(child_width, 0));
		{
	...
	else
	{
		...
		ImGui::BeginChild(crypt_str("##LUA_SECOND"), ImVec2(child_width, 0));
		{
...
void c_menu::draw_radar(int child)
{
	if (!child)
	{
		...
		ImGui::BeginChild(crypt_str("##RADAR_FIRST"), ImVec2(child_width, 0));
		{
...
void c_menu::draw_player_list(int child)
{
	...
	if (!child)
	{
		...
		ImGui::BeginChild(crypt_str("##ESP2_FIRST"), ImVec2(child_width, 0));
		{
	...
	else
	{
		...
		tab_start();
		ImGui::BeginChild(crypt_str("##ESP1_SECOND"), ImVec2(child_width, 0));
		{
...
void c_menu::draw(bool is_open)
{
	...
	// start menu render
	ImGui::Begin(crypt_str("elleqtcheats"), nullptr, ImGuiWindowFlags_NoDecoration);
	{
		...
		if (update_dpi)
		{
			...
			// setup new elements size
			tabs_width = width * 0.206;
			tabs_height = height * (1 - 0.105);

			footer_width = width;
			footer_height = height * 0.105;

			category_width = width * 0.794;
			category_height = height * 0.125;

			content_width = width * 0.794;
			content_height = height * (1 - 0.125 - 0.105);

			child_width = width * 0.306;
		...
		ImGui::BeginGroup();
		ImGui::PushStyleColor(ImGuiCol_ChildBg, ImVec4(51 / 255.f, 50 / 255.f, 64 / 255.f, m_alpha));
		ImGui::BeginChild(crypt_str("##tabs_area"), ImVec2(tabs_width, tabs_height)); // there will be tabs
		{
			ImGui::PushFont(root);
			...
			if (ImGui::MainTab(crypt_str("Player list"), ico_bottom, crypt_str("P"), active_tab == 6, ImVec2(124 * dpi_scale, 0)) && last_tab == active_tab && !preview_reverse) active_tab = 6;
		...
		ImGui::SameLine();
		ImGui::SetCursorPosX(ImGui::GetCursorPosX() - styles.ItemSpacing.x);
		ImGui::BeginGroup();
		...
		ImGui::BeginChild(crypt_str("##categories"), ImVec2(category_width, category_height));
		{
			...
			default: draw_tabs_default(); break;
			}
		...
		ImGui::BeginGroup();  // groupping two childs to prevent multipadding
		{
			...
			{
				ImGui::BeginChild(crypt_str("##content_first"), ImVec2(child_width, 0)); // first functional child
				{
					ImGui::PushStyleVar(ImGuiStyleVar_Alpha, (1.f - preview_alpha) * m_alpha);
				...
				ImGui::BeginChild(crypt_str("##content_second"), ImVec2(child_width, 0)); // second functional child
				{
					ImGui::PushStyleVar(ImGuiStyleVar_Alpha, (1.f - preview_alpha) * m_alpha);
		...
		// Fill bottom
		ImGui::SetCursorPos(ImGui::GetWindowPos() + ImVec2(0, y - footer_height));
		ImGui::PushStyleColor(ImGuiCol_ChildBg, ImVec4(39 / 255.f, 38 / 255.f, 48 / 255.f, m_alpha));
		ImGui::BeginChild("##footer", ImVec2(x, footer_height));
		ImGui::PushFont(root_bold);

		ImGui::SetCursorPos(ImGui::GetCursorPos() + ImVec2(15 * dpi_scale , (footer_height / 2 * dpi_scale) - (ImGui::CalcTextSize(crypt_str("elleqtcheats")).y / 2)));

		ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(1.f, 1.f, 1.f, 1.f));
		ImGui::PushStyleVar(ImGuiStyleVar_Alpha, m_alpha);
		ImGui::Text(crypt_str("elleqtcheats"));
		ImGui::PopStyleVar();
		ImGui::PopStyleColor();
