...
{
	if (!p_localize)
		p_localize = get_interface< ILocalize >(crypt_str("engine.dll"), crypt_str("VEngineVGui"));

	return p_localize;
}

IEngineVGui* C_CSGO::m_enginevgui()
{
	if (!p_enginevgui)
		p_enginevgui = get_interface<IEngineVGui>(crypt_str("localize.dll"), crypt_str("Localize_"));

	return p_enginevgui;
}

ISoundServices* C_CSGO::m_soundservices() {
	if (!p_soundservices)
		p_soundservices = **reinterpret_cast<ISoundServices***>(util::find_pattern(crypt_str("engine.dll"), "\xB9\x00\x00\x00\x00\x50\xA1\x00\x00\x00\x00\x57\xFF\x36", "x????xx????xxx"));

	return p_soundservices;
}
