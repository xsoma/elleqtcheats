...
namespace util
{
	...
	void copy_command(CUserCmd* cmd, int tickbase_shift)
	{
		...
		do
		{
			...
			if (command->m_tickcount != INT_MAX && m_clientstate()->m_iDeltaTick > 0)
				m_prediction()->Update(m_clientstate()->m_iDeltaTick, true, m_clientstate()->m_iLastCommandAck, m_clientstate()->m_iLastOutgoingCommand + m_clientstate()->m_iChockedCommands);

			command->m_command_number = sequence_number;
			command->m_predicted = command->m_tickcount != INT_MAX;

			++m_clientstate()->m_iChockedCommands; //-V807

			if (m_clientstate()->m_ptrNetChannel)
			{
				++m_clientstate()->m_ptrNetChannel->m_nChokedPackets;
				++m_clientstate()->m_ptrNetChannel->m_nOutSequenceNr;
			}
