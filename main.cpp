...
__forceinline void setup_netvars()
{
	...
	while (client)
	{
		...
		if (recvTable)
			netvars::get().tables.emplace(client->m_pNetworkName, recvTable);
...
__forceinline void setup_hooks()
{
	...
	//hooks::enginevgui_hook = new vmthook(reinterpret_cast<DWORD**>(m_enginevgui()));
	//hooks::enginevgui_hook->hook_function(reinterpret_cast<uintptr_t>(hooks::enginevgui_hook), 14); //-V107 //-V221
	...
	//hooks::soundservice_hook = new vmthook(reinterpret_cast<DWORD**>(m_soundservices()));
	//hooks::soundservice_hook->hook_function(reinterpret_cast<uintptr_t>(hooks::hooked_onsoundstart), 27);
