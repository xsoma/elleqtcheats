...
bool entity_t::is_player()
{
	...
	return call_virtual<bool(__thiscall*)(void*)>(this, 157)(this);
}
...
bool player_t::setup_bones_fixed(matrix3x4_t* matrix, int mask)
{
	...
	*(uint8_t*)((uintptr_t)this + 0x274) = 0;
	...
	if (mask == BONE_USED_BY_ANYTHING)
		setuped = SetupBones(matrix, MAXSTUDIOBONES, mask, m_flSimulationTime());
	else
	{
...
#else
	setuped = SetupBones(matrix, matrix ? MAXSTUDIOBONES : -1, mask, m_flSimulationTime());
#endif
