...
namespace hooks
{
	...
	void GUI_Init(IDirect3DDevice9* pDevice)
	{
		...
		// Menu fonts
		c_menu::get().root_small = io.Fonts->AddFontFromMemoryTTF((void*)pt_root_regular, sizeof(pt_root_regular), 15.f, &m_config, ranges);
		c_menu::get().root = io.Fonts->AddFontFromMemoryTTF((void*)pt_root_regular, sizeof(pt_root_regular), 16.f, &m_config, ranges);
		c_menu::get().root_large = io.Fonts->AddFontFromMemoryTTF((void*)pt_root_bold, sizeof(pt_root_bold), 18.f, &m_config, ranges);
		c_menu::get().root_bold = io.Fonts->AddFontFromMemoryTTF((void*)pt_root_bold, sizeof(pt_root_bold), 15.f, &m_config, ranges);

		// Icon fonts
		c_menu::get().ico_menu = io.Fonts->AddFontFromMemoryTTF((void*)icomenu, sizeof(icomenu), 17.f, &m_config, io.Fonts->GetGlyphRangesCyrillic());
		c_menu::get().ico_bottom = io.Fonts->AddFontFromMemoryTTF((void*)iconbot, sizeof(iconbot), 17.f, &m_config, io.Fonts->GetGlyphRangesCyrillic());
