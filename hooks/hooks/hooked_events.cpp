...
void C_HookedEvents::FireGameEvent(IGameEvent* event)
{
	...
	else if (!strcmp(event_name, crypt_str("player_death")))
	{
		...
		if (user_id == m_engine()->GetLocalPlayer())
			walkbot::get().marker = walkbot::get().prishel = 0;
		else if (g_ctx.local()->is_alive() && attacker_id == m_engine()->GetLocalPlayer())
		{
	...
	else if (!strcmp(event_name, crypt_str("player_hurt")))
	{
		...
		if (attacker_id == m_engine()->GetLocalPlayer() && user_id != m_engine()->GetLocalPlayer())
		{
			...
			if (current_shot)
			{
				...
				if (m_engine()->GetPlayerInfo(current_shot->last_target, &player_info) && !player_info.fakeplayer)
				{
					// find the right record
					for (auto it = lagcompensation::get().player_sets.begin(); it != lagcompensation::get().player_sets.end(); ++it)
						if (it->id == player_info.steamID64 && strstr(current_shot->shot_info.server_hitbox.c_str(), current_shot->shot_info.client_hitbox.c_str())) // did we actually hitted right hitbox?
						{
							auto is_head = !strcmp(current_shot->shot_info.server_hitbox.c_str(), "Head");

							if (((resolver_side)current_shot->side == RESOLVER_FIRST || (resolver_side)current_shot->side == RESOLVER_SECOND))
							{
								if (it->res_type != HISTORY_DEFAULT)
								{
									if (is_head)
									{
										// we can say that he is using desync
										it->res_type = HISTORY_DEFAULT;
										it->faking = true;
									}

									// some variables to bruteforce
									if ((resolver_side)current_shot->side == RESOLVER_FIRST)
										it->pos += 1;
									else
										it->neg += 1;
							...
							{
								// he might using low delta?
								if ((resolver_side)current_shot->side == RESOLVER_LOW_FIRST || (resolver_side)current_shot->side == RESOLVER_LOW_SECOND)
								{
									it->res_type = HISTORY_LOW;

									if ((resolver_side)current_shot->side == RESOLVER_LOW_FIRST)
										it->pos += 1;
									else
										it->neg += 1;
								...
								else if ((resolver_side)current_shot->side == RESOLVER_ORIGINAL)
								{
									if (it->res_type != HISTORY_ORIGINAL)
									{
										it->res_type = HISTORY_ORIGINAL;
										it->faking = false;

										break;
									}
								}
								else if ((resolver_side)current_shot->side == RESOLVER_ZERO)
								{
									if (it->res_type != HISTORY_ZERO)
									{
										it->res_type = HISTORY_ZERO;
										break;
									}
								}
							}
