...
bool __stdcall hooks::hooked_createmove(float smt, CUserCmd* m_pcmd)
{
	...
	if (key_binds::get().get_key_bind_state(KB_EDGEJUMP) && engineprediction::get().backup_data.flags & FL_ONGROUND && !(g_ctx.local()->m_fFlags() & FL_ONGROUND)) //-V807
		m_pcmd->m_buttons |= IN_JUMP;

	if (key_binds::get().get_key_bind_state(KB_SLOWWALK))
	...
	if (g_cfg.ragebot.enable && !g_ctx.globals.weapon->is_non_aim() && engineprediction::get().backup_data.flags & FL_ONGROUND && g_ctx.local()->m_fFlags() & FL_ONGROUND)
		slowwalk::get().create_move(m_pcmd, 0.95f + 0.003125f * (16 - m_clientstate()->m_iChockedCommands));
	...
	if (m_gamerules()->m_bIsValveDS() && m_clientstate()->m_iChockedCommands >= 6) //-V648
	{
		g_ctx.send_packet = true;
		fakelag::get().started_peeking = false;
	}
	else if (!m_gamerules()->m_bIsValveDS() && m_clientstate()->m_iChockedCommands >= 16) //-V648
	{
		g_ctx.send_packet = true;
		fakelag::get().started_peeking = false;
	}
	...
	if (g_ctx.globals.should_choke_packet && !(m_pcmd->m_buttons & IN_ATTACK))
	{
	...
	if (g_ctx.send_packet && m_clientstate()->m_ptrNetChannel)
	{
		auto choked_packets = m_clientstate()->m_ptrNetChannel->m_nChokedPackets;
	...
	correct.choked_commands = m_clientstate()->m_iChockedCommands + 1;
	...
	if (!g_ctx.send_packet && !m_gamerules()->m_bIsValveDS())
	{
		...
		{
			const auto backup_choke = net_channel->m_nChokedPackets;
	...
	if (g_cfg.misc.buybot_enable && g_ctx.globals.should_buy)
	{
		...
		if (!g_ctx.globals.should_buy)
		{
			...
			if (g_cfg.misc.buybot3[BUY_GRENADES])
				buy += crypt_str("buy molotov; buy hegrenade; buy smokegrenade; buy incgrenade;");
