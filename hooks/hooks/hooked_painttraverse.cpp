...
void __fastcall hooks::hooked_painttraverse(void* ecx, void* edx, vgui::VPANEL panel, bool force_repaint, bool allow_force)
{
	...
	static auto con_filter_text = m_cvar()->FindVar(crypt_str("con_filter_text"));
	...
	if (set_console)
	{
		...
		con_filter_text->SetValue(crypt_str(""));
	...
	if (log_value != g_cfg.misc.show_default_log)
	{
		...
		if (log_value)
			...
			con_filter_text->SetValue(crypt_str("IrWL5106TZZKNFPz4P4Gl3pSN?J370f5hi373ZjPg%VOVh6lN"));
	}
